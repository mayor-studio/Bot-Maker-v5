...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        ...
        .setDescription('Set up the role deletion protection system')
        ...
        .addIntegerOption(option =>
            option.setName('limit')
                .setDescription('Allowed number of role deletions per day')
                .setRequired(true)
        ),
    ...
    async execute(interaction) {
        ...
        try {
            ...
            return interaction.editReply({
                content: `✅ **The role deletion protection system has been set to \`${status.toUpperCase()}\` with a limit of \`${limit}\` deletions per day.**\nMake sure my role is at the top of the server roles list.`
            });
        } catch (err) {
            console.error(err);
            return interaction.editReply({
