    ...
    if (interaction.isStringSelectMenu()) {
        if(interaction.customId == 'select_bot') {
          ...
          if(selected == "BuyNormalBroadcast") {
            if(buyStatusDB.get(`Broadcast2`) === "off"){
          ...
          if(!normalBroadcastPrice) normalBroadcastPrice = 40;
...
userbalance = parseInt(usersdata.get(`balance_${interaction.user.id}_${interaction.guild.id}`))
        if(userbalance < normalBroadcastPrice) return interaction.reply({content:`**⛔ انت لا تمتلك الرصيد الكافي ، تحتاج الى \`${normalBroadcastPrice}\` عملات**` , ephemeral:true})
