  ...
  async execute(interaction) {
    if (interaction.isStringSelectMenu()) {
      if (interaction.customId == 'select_bot3') {
        ...
        if (selected == "BuyMention") {
          if (buyStatusDB.get(`mention`) === "off") {
          ...
          let BroadcastPrice = parseInt(prices.get(`mention_price_${interaction.guild.id}`));
          ...
          const modal = new ModalBuilder()
            .setCustomId('BuyMention_Modal')
            .setTitle('Make Mention Bot');
