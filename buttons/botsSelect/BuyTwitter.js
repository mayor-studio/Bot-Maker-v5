  ...
  async execute(interaction) {
    if (interaction.isStringSelectMenu()) {
      if (interaction.customId == 'select_bot3') {
        ...
        if (selected == "BuyTwitter") {
          if (buyStatusDB.get(`twitter`) === "off") {
          ...
          let BroadcastPrice = parseInt(prices.get(`twitter_price_${interaction.guild.id}`));
          ...
          const modal = new ModalBuilder()
            .setCustomId('BuyTwitter_Modal')
            .setTitle('Make Twitter Bot');
