  ...
  async execute(interaction) {
    if (interaction.isStringSelectMenu()) {
      if (interaction.customId == 'select_bot3') {
        ...
        if (selected == "BuyWarns") {
          if (buyStatusDB.get(`warns`) === "off") {
            return interaction.reply({
              content: `***لا تستطيع شراء هذا البوت في الوقت الحالي***\n**تستطيع ان تحاول مره ثانيه عندما يكون متوفر**`,
              ephemeral: true
            });
          }
          ...
          let BroadcastPrice = parseInt(prices.get(`warns_price_${interaction.guild.id}`));
          ...
          const modal = new ModalBuilder()
            .setCustomId('BuyWarns_Modal')
            .setTitle('Make Warns Bot');
