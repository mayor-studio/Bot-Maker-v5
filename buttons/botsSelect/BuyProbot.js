  ...
  async execute(interaction){
    if (interaction.isStringSelectMenu()) {
        if(interaction.customId == 'select_bot') {
          ...
          if(selected == "BuyProbot") {
            if(buyStatusDB.get(`probot`) === "off"){
          ...
          let probotPrice = parseInt(prices.get(`probot_price_${interaction.guild.id}`))
          if(!probotPrice) probotPrice = 40;
...
userbalance = parseInt(usersdata.get(`balance_${interaction.user.id}_${interaction.guild.id}`))
        if(userbalance < probotPrice) return interaction.reply({content:`**⛔ انت لا تمتلك الرصيد الكافي ، تحتاج الى \`${probotPrice}\` عملات**` , ephemeral:true})
           const modal = new ModalBuilder()
          .setCustomId('BuyProbot_Modal')
       .setTitle('Make Probot Premium Bot');
