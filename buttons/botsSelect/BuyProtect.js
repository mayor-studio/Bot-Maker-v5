    ...
    if (interaction.isStringSelectMenu()) {
        if(interaction.customId == 'select_bot') {
          ...
          if(selected == "BuyProtect") {
            if(buyStatusDB.get(`protect`) === "off"){
          ...
          if(!protectPrice) protectPrice = 40;
...
userbalance = parseInt(usersdata.get(`balance_${interaction.user.id}_${interaction.guild.id}`))
        if(userbalance < protectPrice) return interaction.reply({content:`**⛔ انت لا تمتلك الرصيد الكافي ، تحتاج الى \`${protectPrice}\` عملات**` , ephemeral:true})
