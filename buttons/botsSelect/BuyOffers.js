  ...
  async execute(interaction) {
    if (interaction.isStringSelectMenu()) {
      if (interaction.customId == 'select_bot2') {
        ...
        if (selected == "BuyOffers") {
          if (buyStatusDB.get(`offers`) === "off") {
          ...
          let BroadcastPrice = parseInt(prices.get(`offers_price_${interaction.guild.id}`));
          ...
          const modal = new ModalBuilder()
            .setCustomId('BuyOffers_Modal')
            .setTitle('Make Offers Bot');
