  ...
  async execute(interaction){
    if (interaction.isStringSelectMenu()) {
        if(interaction.customId == 'select_bot3') {
          ...
          if(selected == "BuyColor") {
            if(buyStatusDB.get(`color`) === "off"){
          ...
          let colorPrice = parseInt(prices.get(`color_price_${interaction.guild.id}`))
          if(!colorPrice) colorPrice = 70;
...
userbalance = parseInt(usersdata.get(`balance_${interaction.user.id}_${interaction.guild.id}`))
        if(userbalance < colorPrice) return interaction.reply({content:`**⛔ انت لا تمتلك الرصيد الكافي ، تحتاج الى \`${colorPrice}\` عملات**` , ephemeral:true})
           const modal = new ModalBuilder()
          .setCustomId('BuyColor_Modal')
       .setTitle('Make Color Bot');
