  ...
  async execute(interaction){
    if (interaction.isStringSelectMenu()) {
        if(interaction.customId == 'select_bot') {
          ...
          if(selected == "BuyTicket") {
            if(buyStatusDB.get(`ticket`) === "off"){
          ...
          let ticketPrice = parseInt(prices.get(`ticket_price_${interaction.guild.id}`))
          if(!ticketPrice) ticketPrice = 160;
...
userbalance = parseInt(usersdata.get(`balance_${interaction.user.id}_${interaction.guild.id}`))
        if(userbalance < ticketPrice) return interaction.reply({content:`**⛔ انت لا تمتلك الرصيد الكافي ، تحتاج الى \`${ticketPrice}\` عملات**` , ephemeral:true})
           const modal = new ModalBuilder()
          .setCustomId('BuyTicket_Modal')
       .setTitle('Make Ticket Bot');
