...
module.exports = {
  ...
  async execute(interaction) {
    if (interaction.isModalSubmit()) {
      if (interaction.customId == "BuyTwitter_Modal") {
        ...
        try {
          ...
          let price1 = prices.get(`twitter_price_${interaction.guild.id}`) || 70;
          ...
          // Calculate newbalance before using it
          ...
          client29.on("ready", async () => {
            let doneembeduser = new EmbedBuilder()
              ...
              .addFields(
                ...
                { name: `**نوع البوت**`, value: `**\`تويتر\`**`, inline: false },
            ...
            const thebut = new ButtonBuilder()
              ...
              .setURL(`https://discord.com/api/oauth2/authorize?client_id=${client29.user.id}&permissions=8&scope=bot%20applications.commands`);
          ...
          let doneembedprove = new EmbedBuilder()
            ...
            .setDescription(`**تم شراء بوت بواسطة : ${interaction.user}**`)
            .addFields(
                {name: '👤 المشتري', value: `${interaction.user.username}`, inline: true},
                {name: '🤖 نوع البوت', value: 'تويتر', inline: true}, 
                {name: '💰 السعر', value: `${price1}`, inline: true},
                {name: '💳 رصيد المشتري', value: `${newbalance}`, inline: true}
            )
                    .setImage(interaction.guild.banner ? interaction.guild.bannerURL({ dynamic: true, size: 1024 }) : null)
                    .setFooter({ text: `Developed by ${interaction.guild.name}`, iconURL: interaction.guild.iconURL({ dynamic: true }) })            .setTimestamp();
          ...
          require("../../Bots/twitter/handlers/events")(client29);
          require("../../events/requireBots/twitter-commands")(client29);
          const folderPath = path.resolve(__dirname, '../../Bots/twitter/slashcommand29');
          ...
          const folderPath3 = path.resolve(__dirname, '../../Bots/twitter/handlers');
          for (let file of readdirSync(folderPath3).filter(f => f.endsWith('.js'))) {
            const event = require(path.join(folderPath3, file))(client29);
          }
  const herc = new Hercai();

  client29.on('messageCreate', async message => {
    const chan = twitterDB.get(`twitter_room_${message.guild.id}`);
        ...
        await message.reply('عذرًا، لقد واجهت بعض المشاكل أثناء محاولة الرد، حاول مرة أخرى!');
          ...
          client29.on("interactionCreate", async (interaction) => {
            if (interaction.customId === "help_general") {
              ...
              const embed = new EmbedBuilder()
                ...
                .setTitle('قائمة اوامر البوت')
                .addFields(
                  { name: `\`/set-twitter-room\``, value: `لتحديد روم تويتر` }
                )
                .setTimestamp()
                .setFooter({ text: `Requested By ${interaction.user.username}`, iconURL: interaction.user.displayAvatarURL({ dynamic: true }) })
                .setColor('DarkButNotBlack');
          ...
          client29.on('ready', async () => {
            setInterval(async () => {
              ...
              if (thiss) {
                if (thiss.timeleft <= 0) {
                  console.log(`${client29.user.id} Ended`);
                  await client29.destroy();
                }
              }
            }, 1000);
          });
          ...
          const folderPath2 = path.resolve(__dirname, '../../Bots/twitter/events');
          ...
          client29.on("interactionCreate", async (interaction) => {
            if (interaction.isChatInputCommand()) {
              ...
              const command = client29.twitterSlashCommands.get(interaction.commandName);
          ...
          await client29.login(Bot_token).catch(async () => {
            return interaction.editReply({ content: `**فشل التحقق , الرجاء تفعيل اخر ثلاث خيارات في قائمة البوت**` });
          });

          if (!twitter) {
            await tokens.set(`twitter`, [{ token: Bot_token, prefix: Bot_prefix, clientId: client29.user.id, owner: interaction.user.id, timeleft: 2629744 }]);
          } else {
            await tokens.push(`twitter`, { token: Bot_token, prefix: Bot_prefix, clientId: client29.user.id, owner: interaction.user.id, timeleft: 2629744 });
          }
