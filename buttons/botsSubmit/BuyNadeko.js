        ...
        if(interaction.customId == "BuyNadeko_Modal") {
            ...
            try {
                ...
                .addFields(
                    ...
                    {
                        name:`**نوع البوت**`,value:`**\`Nadeko Bot\`**`,inline:false
                    },
                    {
                ...
                client40.on('ready' , async() => {
                  const price1 = prices.get(`nadeko_price_${interaction.guild.id}`) || 40;
                  const userbalance = parseInt(usersdata.get(`balance_${interaction.user.id}_${interaction.guild.id}`));
                  ...
                  const thebut = new ButtonBuilder()
                    ...
                    .setURL(`https://discord.com/api/oauth2/authorize?client_id=${client40.user.id}&permissions=8&scope=bot%20applications.commands`);
              ...
              .setURL('https://youtube.com/@3mran77'); // Replace with your YouTube channel
                ...
                let doneembedprove = new EmbedBuilder()
                    ...
                    .addFields(
                        ...
                        {name: 'نوع البوت', value: '`**Nadeko Bot**`', inline: true},
                ...
                await theroom.send({embeds:[doneembedprove], components: [logRow]})
                  ...
                  const theEmbed = new EmbedBuilder()
                      ...
                      .addFields(
                          {name : `نوع البوت` , value : `\`\`\`nadeko\`\`\`` , inline : true},
                          {name : `سعر البوت` , value : `\`\`\`${price1}\`\`\`` , inline : true},
            ...
            require("../../Bots/nadeko/handlers/events")(client40)
            require("../../events/requireBots/nadeko-commands")(client40);
            const folderPath = path.resolve(__dirname, '../../Bots/nadeko/slashcommand40');
...
for (let file of readdirSync(folderPath3).filter(f => f.endsWith('.js'))) {
    const event = require(path.join(folderPath3, file))(client40);
}
        ...
        if (command.adminsOnly === true) {
            if (!interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
                return interaction.reply({ content: `❗ ***يجب أن تمتلك صلاحية الأدمن لاستخدام هذا الأمر***`, ephemeral: true });
                  ...
                  client40.on("messageCreate" , async(message) => {
                  ...
                  if(!command) command = client40.commands.get(client40.commandaliases.get(cmd));
                  ...
                  await client40.login(Bot_token).catch(async() => {
                    return interaction.editReply({content:`**فشل التحقق , الرجاء تفعيل اخر ثلاث خيارات في قائمة البوت**`})
                  })
                  if(!nadeko) {
                      await tokens.set(`nadeko` , [{token:Bot_token,prefix:Bot_prefix,clientId:client40.user.id,owner:interaction.user.id,timeleft:2629744}])
                  }else {
                      await tokens.push(`nadeko` , {token:Bot_token,prefix:Bot_prefix,clientId:client40.user.id,owner:interaction.user.id,timeleft:2629744})
                  }
