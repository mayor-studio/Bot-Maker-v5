  ...
  */
  async execute(interaction){
    if (interaction.isModalSubmit()) {
        if(interaction.customId == "BuyColor_Modal") {
            ...
            try{
                ...
                let price1 = prices.get(`color_price_${interaction.guild.id}`) || 70;
...
const { Routes } = require('discord-api-types/v10');
               client25.on("ready" , async() => {
                ...
                .addFields(
                    ...
                    {
                        name:`**نوع البوت**`,value:`**\`بيع الوان\`**`,inline:false
                    },
                    {
                ...
                const thebut = new ButtonBuilder()
                    ...
                    .setURL(`https://discord.com/api/oauth2/authorize?client_id=${client25.user.id}&permissions=8&scope=bot%20applications.commands`);
              ...
              .setURL('https://youtube.com/@3mran77'); // Replace with your YouTube channel
                ...
                let doneembedprove = new EmbedBuilder()
                    ...
                    .addFields(
                        ...
                        {name: 'نوع البوت', value: '`الالوان`', inline: true},
                ...
                await theroom.send({embeds:[doneembedprove], components: [logRow]})
                  ...
                  const theEmbed = new EmbedBuilder()
                                              ...
                                              .addFields(
                                                  {name : `نوع البوت` , value : `\`\`\`بيع الوان\`\`\`` , inline : true},
                                                  {name : `سعر البوت` , value : `\`\`\`${price1}\`\`\`` , inline : true},
                ...
                require("../../Bots/color/handlers/events")(client25)
                require("../../events/requireBots/color-commands")(client25);
                const folderPath = path.resolve(__dirname, '../../Bots/color/slashcommand25');
...
const folderPath3 = path.resolve(__dirname, '../../Bots/color/handlers');
...
client25.on("ready" , async() => {
    ...
    const folderPath2 = path.resolve(__dirname, '../../Bots/color/events');
...
client25.on("interactionCreate", async (interaction) => {
  ...
  await client25.login(Bot_token).catch(async() => {
    return interaction.editReply({content:`**فشل التحقق , الرجاء تفعيل اخر ثلاث خيارات في قائمة البوت**`})
                  ...
                  if(!color) {
                      await tokens.set(`color` , [{token:Bot_token,prefix:Bot_prefix,clientId:client25.user.id,owner:interaction.user.id,timeleft:2629744}])
                  }else {
                      await tokens.push(`color` , {token:Bot_token,prefix:Bot_prefix,clientId:client25.user.id,owner:interaction.user.id,timeleft:2629744})
                  }
