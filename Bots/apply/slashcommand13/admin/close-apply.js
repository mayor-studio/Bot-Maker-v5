...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    .setName('close-apply')
    .setDescription('Close the currently open bot application'),
  ...
  async execute(interaction, client) {
    ...
    if (!apply) {
      embed1.setTitle(`❌ No application is currently open`);
      return interaction.editReply({ embeds: [embed1] });
    }

    await applyDB.delete(`apply_${interaction.guild.id}`);

    embed1.setTitle(`✅ Bot application has been successfully closed`);
    return interaction.editReply({ embeds: [embed1] });
  }
};
