const { ButtonStyle, ButtonBuilder, EmbedBuilder, ActionRowBuilder, StringSelectMenuBuilder } = require("discord.js");
const config = require("./config");
const has_play = new Map();
config.numbers = config.numbers.slice(1);
const canvas = require("canvas");
canvas.registerFont("./Bots/games/handlers/hosam.ttf", { family: "Arial" });

/**
 * Keeps track of kicked players in each guild
 * { [guildId]: [{id, username, avatar}] }
 */
...
async function roulette_command(message) {
  ...
  kickedPlayers.set(message.guild.id, []);
...
`)
  ...
  let row = new ActionRowBuilder()
    .addComponents(
      createButton("SUCCESS", `join_roulette`, `دخول إلي اللعبة`),
      createButton(`DANGER`, `left_roulette`, `اخرج من اللعبة`)
    );
  let row_2 = new ActionRowBuilder()
    .addComponents(
      createButton("SUCCESS", `join_roulette`, `دخول إلي اللعبة`, null, true),
      createButton(`DANGER`, `left_roulette`, `اخرج من اللعبة`, null, true)
    );
...
async function roulette(message) {
  ...
  // revive button (always visible, but enabled only if there are kicked members)
  let reviveRow = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId('revive_member')
      .setLabel('🔄 إرجاع لاعب')
      .setStyle(ButtonStyle.Primary)
      .setDisabled(!kickedPlayers.get(guildId) || kickedPlayers.get(guildId).length === 0)
  );

  let msg = await message.channel.send({
    content: `<@${winner.id}> لديك **30 ثانية** لإختيار لاعب لطرده${reviveRow.components[0].data.disabled ? '' : ' أو إعادة أحد المطرودين'}`,
    components: [...createMultipleButtons(buttons_array, winner), reviveRow]
  });

  let kickedThisRound = null;
  let reviveMenuMsg = null;

  // Button collector for main kick/leave/revive
  const filter = i =>
    (i.customId === 'revive_member' && (!kickedPlayers.get(guildId) || kickedPlayers.get(guildId).length > 0)) ||
    (buttons_array.some(b => b.id === i.customId) && i.user.id == winner.id);
  let collector = msg.createMessageComponentCollector({ filter, time: 30000 });

  collector.on("collect", async inter => {
    if (!has_play.get(message.guild.id)) return;
    if (inter.customId === "revive_member") {
      // Revive select menu
      let kicked = kickedPlayers.get(guildId) || [];
      if (!kicked.length) return inter.reply({ content: 'لا يوجد لاعبين مطرودين لإرجاعهم!', ephemeral: true });

      let select = new StringSelectMenuBuilder()
        .setCustomId('revive_select')
        .setPlaceholder('اختر لاعب لإرجاعه')
        .addOptions(
          kicked.map(p => ({
            label: p.username,
            value: p.id,
            description: `إرجاع ${p.username}`,
            emoji: '🟢'
          }))
        );

      let selectRow = new ActionRowBuilder().addComponents(select);
      reviveMenuMsg = await inter.reply({
        content: 'اختر لاعباً لإرجاعه إلى اللعبة:',
        components: [selectRow],
        ephemeral: true
      });
    } else {
      // Handle kick/leave logic
      collector.stop('kick');
      let choice = inter.customId;
      if (choice === winner.id) {
        await inter.deferUpdate();
        kickedThisRound = winner;
        message.channel.send({ content: `💣 | لقد انسحب <@${winner.id}> من اللعبة ، سيتم بدء الجولة القادمة خلال 6 ثواني...` });
      } else {
        await inter.deferUpdate();
        kickedThisRound = data.players.find(p => p.id == choice);
        message.channel.send({ content: `💣 | تم طرد <@${choice}> من اللعبة ، سيتم بدء الجولة القادمة خلال 6 ثواني...` });
      }
    }
  });

  // Handle select menu for revive
  msg.client.on('interactionCreate', async inter => {
    if (!inter.isStringSelectMenu() || inter.customId !== 'revive_select') return;
    if (!has_play.get(message.guild.id)) return;
    let reviveId = inter.values[0];
    let kicked = kickedPlayers.get(guildId) || [];
    let revived = kicked.find(p => p.id == reviveId);
    if (!revived) return inter.reply({ content: 'هذا اللاعب غير موجود في قائمة المطرودين!', ephemeral: true });

    // إرجاع اللاعب
    let data = has_play.get(message.guild.id);
    data.players.push({ ...revived, position: data.players.length, color: config.roulette_colors[data.players.length % config.roulette_colors.length] });
    has_play.set(message.guild.id, data);
    kickedPlayers.set(guildId, kicked.filter(p => p.id != reviveId));
    await inter.update({
      content: `✅ تم إرجاع <@${reviveId}> إلى اللعبة!`,
      components: [],
      ephemeral: true
    });
    // Update revive button status for next round
    await msg.edit({
      components: [
        ...
        new ActionRowBuilder().addComponents(
          new ButtonBuilder()
            ...
            .setLabel('🔄 إرجاع لاعب')
            .setStyle(ButtonStyle.Primary)
            .setDisabled(kickedPlayers.get(guildId).length === 0)
        )
      ]
    }).catch(() => 0);
  });
  ...
  collector.on("end", async (collected, reason) => {
    // Disable all buttons
    let disabledButtons = buttons_array.map(e => ({ ...e, disabled: true }));
    let disabledReviveRow = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId('revive_member')
        .setLabel('🔄 إرجاع لاعب')
        .setStyle(ButtonStyle.Primary)
    ...
    await msg.edit({ components: [...createMultipleButtons(disabledButtons, winner), disabledReviveRow] }).catch(() => 0);

    // If revive select menu was open, disable it
    if (reviveMenuMsg) {
      try {
        await reviveMenuMsg.edit({ components: [] }).catch(() => 0);
      } catch {}
    }
    if (!kickedThisRound) {
      kickedThisRound = winner;
      message.channel.send({ content: `💣 | تم طرد <@${winner.id}> من اللعبة لعدم تفاعله ، سيتم بدء الجولة القادمة خلال 6 ثواني...` });
    }
    // Add to kicked list if not already in
    if (kickedThisRound && kickedThisRound.id !== winner.id) {
      let kicked = kickedPlayers.get(guildId) || [];
      if (!kicked.find(p => p.id === kickedThisRound.id)) {
        kicked.push(kickedThisRound);
        kickedPlayers.set(guildId, kicked);
      }
    }
    // Remove player from current list
    let data = has_play.get(message.guild.id);
    let idx = data.players.findIndex(p => p.id == kickedThisRound.id);
    if (idx !== -1) {
      data.players.splice(idx, 1);
      has_play.set(message.guild.id, data);
    }

    // Wait 6s before next round
    await sleep(6000);
    roulette(message);
  });
}
