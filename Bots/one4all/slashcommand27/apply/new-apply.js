...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addRoleOption((option) =>
      option.setName("role")
        .setDescription("The role the application is for")
        .setRequired(true)
    )
    .addStringOption((option) =>
      option.setName("ask1").setDescription("First question").setRequired(true)
    )
    .addStringOption((option) =>
      option.setName("ask2").setDescription("Second question").setRequired(false)
    )
    .addStringOption((option) =>
      option.setName("ask3").setDescription("Third question").setRequired(false)
    )
    .addStringOption((option) =>
      option.setName("ask4").setDescription("Fourth question").setRequired(false)
    )
    .addStringOption((option) =>
      option.setName("ask5").setDescription("Fifth question").setRequired(false)
    )
    .addAttachmentOption((option) =>
      option.setName("image").setDescription("Image for the application embed").setRequired(false)
    )
    .addStringOption((option) =>
  ...
  async execute(interaction, client) {
    ...
    if (!settings) {
      return interaction.reply({
        content: "**Please set up the application system first using /setup-apply**",
        ephemeral: true,
      });
    }
    ...
    const messageInput = new TextInputBuilder()
      ...
      .setLabel("Embed message content")
