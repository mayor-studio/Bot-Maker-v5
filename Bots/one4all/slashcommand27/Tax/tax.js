...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('tax')
        .setDescription('Get the tax of a number')
        .addStringOption(option => 
            option
                .setName('number')
                .setDescription('The number to calculate tax for')
                .setRequired(true)),
    async execute(interaction) {
        ...
        let tax = Math.floor(number2 * (20) / (19) + 1); // amount with tax
        let tax2 = Math.floor(tax - number2); // tax amount
